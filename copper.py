...
st.set_page_config(page_title='Industrial Copper Modeling',initial_sidebar_state='expanded',layout='wide')
...
class option():
    ...
    # Mapping Value
    status_dict={'Won':0, 'Draft':1, 'To be approved':2, 'Lost':3, 'Not lost for AM':4,
    'Wonderful':5, 'Revised':6, 'Offered':7, 'Offerable':8}
    item_type_value = ['W', 'WI', 'S', 'Others', 'PL', 'IPL', 'SLAWR']
    # Encoded value
    item_dict ={'W':5.0, 'WI':6.0, 'S':3.0, 'Others':1.0, 'PL':2.0, 'IPL':0.0, 'SLAWR':4.0}

    product_value = [611728, 611733, 611993, 628112, 628117, 628377, 640400, 640405, 640665,
            164141591, 164336407, 164337175, 929423819, 1282007633, 1332077137,
...
select=option_menu('',options=["Selling Price","Status"],
                                icons=["currency-dollar", "check-circle"],
...
# Selling Price prediction data
    with st.form('prediction'):
        ...
        with col1:
            ...
            customer=st.number_input(label='Customer ID', min_value = 12458.0, max_value= 2147483650.0, value=12458.00)
            ...
            item_type=st.selectbox(label='Item Type',options=option.item_type_value)
        ...
        with col2:
            ...
            product_ref=st.selectbox(label='Product Ref',options=option.product_value)

            quantity_log = st.number_input(label='Quantity',min_value=0.00001,max_value=1000000000.0,value=1.0)
    ...
    if button:
        # Not filling all the columns Command
        if not all([item_date, delivery_date, country, item_type, application, product_ref,
            ...
            st.error("Please fill all the required columns")
        else:
            ...
            with open('regression_model.pkl','rb') as files:
                ...
                st.subheader(f":violet[Predicted Selling Price :] {selling_price:.2f}")
